...
files = task_dict.get('input').get('normal_bam')
metadata_file_name = task_dict.get('input').get('tumor_bams')
...
experiment_payload = ExperimentPayload(aligned=is_aligned, library_strategy=library_strategy, reference_genome=reference_genome)
song_payload = SongPayload(analysis_id=analysis_id, analysis_type=analysis_type, experiment_payload=experiment_payload)
